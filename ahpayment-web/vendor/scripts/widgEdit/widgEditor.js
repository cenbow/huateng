...
var widgStylesheet = "/assets/common.css";
...
{
	if (widgAutoClean || confirm("你是否想清理你刚才粘贴的内容的HTML样式?"))
	{
		var matchedHead = "";
		var matchedTail = "";
...
widgEditor.prototype.writeDocument = function(documentContent)
{
	...
	var documentTemplate = "<html><head>INSERT:STYLESHEET:END</head><body id='iframeBody'>INSERT:CONTENT:END</body></html>";
	...
	switch (this.action)
	{
		...
		case "link":
			...
			else
			{
				...
				var theURL = prompt("请输入URL链接:", "http://");
		...
		case "image":
			var theImage = prompt("请输入图片的链接地址:", "");
			
			if (theImage != null && theImage != "")
			{
				var theAlt = prompt("请为图片输入穿插的文本内容:", "");
