 ...
 *
 * Java implementation of the Rijndael (AES) block cipher.
 *
 * @author holo
 *
 */
...
public final class Rijndael {
	...
	/**
	 * Encrypt message array with padding.
	 * 
	 * @param message
	...
	public byte[] encryptArray(byte[] message, int offset) throws CryptoException {
		...
		if(numOfPads != BLOCK_SIZE) {
			...
			result = Util.addByteArrays(encryptBlock(last,
					new byte[BLOCK_SIZE]),
					encryptBlock(pads, new byte[BLOCK_SIZE]));
			return result;
		}

		result = Util.addByteArrays(result,
				encryptBlock(last, new byte[BLOCK_SIZE]));

		return result;
	}

	/**
	 * Decrypt message array with padding.
	 * 
	 * @param message
	 * @param offset
	 * @return plain array.
	...
	/**
	 * Encrypt message without padding
	 * 
	 * @param message
	 * @param offset
	 * @return cypher array
	...
	public byte[] encryptArrayNP(byte[] message, int offset) throws CryptoException
	{
		...
		//Number of zero bytes required
		int numOfZeros = BLOCK_SIZE - last.length;
		...
		//add zero to the end of last block
	...
	/**
	 * Decrypt message without padding
	 * @param message
	 * @param offset
	 * @return
	 * @throws CryptoException
	...
	public byte[] decryptArrayNP(byte[] message, int offset) throws CryptoException
	{
        ...
        //If message was not mutliple of BLOCK_SIZE
        if(lengthOfLastPart != 0) {
        ...
        for(int i = 0; i < numOfBlocks; i++) {
            ...
            result = Util.addByteArrays(result,
            		decryptBlock(block, new byte[BLOCK_SIZE]));
        }
        
//        //Now do the last part (or last block if message was multiple of BLOCK_SIZE)
//        byte last[] = new byte[lengthOfLastPart];
//        System.arraycopy(message, numOfBlocks * BLOCK_SIZE,
//        		last, 0, lengthOfLastPart);
//        
//        //Decrypt the last block
//        byte[] tmp = aes.decryptBlock(last, new byte[BLOCK_SIZE]);
//        
//        int numOfPads = tmp[tmp.length - 1];
//        
//        byte[] lastBlock = new byte[BLOCK_SIZE - numOfPads];
