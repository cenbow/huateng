...
public class User extends BaseUser {
    ...
    @Getter
    @Setter
    @Length(max=16,message = "电话号码不能超过16个字符")
    ...
    @Override
    public String toString() {
        return toStringHelper(this).add("id",id).add("nick", name).add("mobile",mobile)
                .add("type",type).add("email",email).add("status",status).omitNullValues().toString();
    }
