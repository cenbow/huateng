...
public class UpYun {
    private final static Logger log = LoggerFactory.getLogger(UpYun.class);
	...
	*/
	private String HttpAction(String method, String uri, byte[] datas, File out_file, Boolean auto){
		...
		try{
			if(datas.length == 11 && (new String(datas,0,11, Charsets.UTF_8)).equals("folder:true")){
				is_folder = true;
				datas = null;
			}
		}catch(Exception e){}
		try {
			...
			log.error("failed to upload to upyun,cause:{}",e);
			return null;
		}
		return result;
	}
